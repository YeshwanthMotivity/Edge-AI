...
class Settings(BaseSettings):
    ...
    ner_confidence_threshold: float = Field(
        ...
        description="Minimum NER confidence score to accept a detection"
    )
    ner_inference_timeout_seconds: float = Field(
        default=3.0, gt=0.0,
        description="Max seconds for NER inference before falling back to regex-only"
    )
    ner_model_name: str = Field(
        default="dbmdz/bert-tiny-historic-multilingual-cased",
        description="HuggingFace model ID for NER"
    )
    enable_inference_cache: bool = Field(
        default=True, description="Enable LRU caching for NER inference to save CPU cycles"
    )
